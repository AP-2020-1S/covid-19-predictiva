...
def rollPlot(ciudad, case='acumulado_dia_activos'):
    ts=data.loc[(data['ciudad_de_ubicaci_n']==ciudad)]  
    ts=ts[['fecha_reporte_web',case]]
    ts=ts.set_index('fecha_reporte_web')
    ts.astype('int64')
...
tsC13=roll('Barranquilla')
rollPlot('Barranquilla')

tsC14=roll('Cartagena de Indias')
rollPlot('Cartagena de Indias')

tsC15=roll('Bogotá D.C.')
rollPlot('Bogotá D.C.')

#Decomposing the ts to find its properties
fig=sm.tsa.seasonal_decompose(tsC11.values,freq=7).plot()

fig=sm.tsa.seasonal_decompose(tsC12.values,freq=7).plot()

fig=sm.tsa.seasonal_decompose(tsC13.values,freq=7).plot()

fig=sm.tsa.seasonal_decompose(tsC14.values,freq=7).plot()

fig=sm.tsa.seasonal_decompose(tsC15.values,freq=7).plot()
...
#Para Medellín
tsC=tsC11['acumulado_dia_activos'].values
stationarity(tsC)

#Para Cali
tsC=tsC12['acumulado_dia_activos'].values
stationarity(tsC)

#Para Barranquilla
tsC=tsC13['acumulado_dia_activos'].values
stationarity(tsC)

#Para Cartagena de Indias
tsC=tsC14['acumulado_dia_activos'].values
stationarity(tsC)

#Para Bogotá D.C.
tsC=tsC15['acumulado_dia_activos'].values
stationarity(tsC)
...
y1 = features_and_target_train[['acumulado_dia_activos']]
